...
interface CourseRow {
  ...
  course_name: string;   credits: number;  letter: string; gpa_points: number;
}
interface DetailData {
  student: { id: number; mssv: string; full_name: string; class_code: string; cohort: string };
  gpa: number; warnings: string[]; courses: CourseRow[];
}

const gradeColor = (l: string) => ({
  A: 'bg-emerald-100 text-emerald-700 border-emerald-200',
  B: 'bg-blue-100 text-blue-700 border-blue-200',
  C: 'bg-amber-100 text-amber-700 border-amber-200',
  D: 'bg-orange-100 text-orange-700 border-orange-200',
  F: 'bg-red-100 text-red-700 border-red-200',
}[l] ?? 'bg-slate-100 text-slate-500 border-slate-200');

const gpaColor = (g: number) =>
    g >= 3.5 ? 'text-emerald-600' : g >= 2.5 ? 'text-blue-600' : g >= 2.0 ? 'text-amber-600' : 'text-red-600';

const gpaLabel = (g: number) => {
  if (g >= 3.5) return { text: 'XUẤT SẮC', cls: 'bg-emerald-100 text-emerald-700' };
  if (g >= 3.0) return { text: 'GIỎI',     cls: 'bg-blue-100 text-blue-700' };
  if (g >= 2.5) return { text: 'KHÁ',      cls: 'bg-sky-100 text-sky-700' };
  if (g >= 2.0) return { text: 'TB',        cls: 'bg-amber-100 text-amber-700' };
  return           { text: 'YẾU',           cls: 'bg-red-100 text-red-700' };
};

function StudentDetail({
                         studentId, onBack,
                       }: { studentId: string | null; onBack: () => void }) {
  ...
  if (loading) return (
      <div className="flex items-center justify-center h-[60vh] gap-3 text-slate-400">
        <Loader2 className="w-6 h-6 animate-spin" /><span>Đang tải hồ sơ...</span>
      </div>
  );
  if (error) return (
      <div className="flex flex-col items-center justify-center h-[60vh] gap-3 text-red-400">
        <XCircle className="w-10 h-10" /><p className="font-semibold">{error}</p>
      </div>
  );
  if (!data) return null;
  ...
  const failCount    = courses.filter(c => c.letter === 'F').length;
  const semMap: Record<string, CourseRow[]> = {};
  for (const c of courses) { (semMap[c.semester] ??= []).push(c); }
      ...
      <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
          ...
          <div className="p-6 flex flex-col sm:flex-row items-start sm:items-center gap-5">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex flex-wrap items-center gap-2 mb-1">
                ...
                <span className={`text-[10px] font-bold px-2.5 py-0.5 rounded-full ${label.cls}`}>{label.text}</span>
              </div>
              <div className="flex flex-wrap gap-x-3 gap-y-1 text-sm text-slate-500">
                ...
                <span>·</span><span>{student.class_code}</span>
                <span>·</span><span>Khóa {student.cohort}</span>
              </div>
            </div>
            <div className="text-right shrink-0">
              <div className={`text-4xl font-black ${gpaColor(gpa)}`}>{gpa.toFixed(2)}</div>
              <div className="text-xs text-slate-400 font-semibold uppercase tracking-wider">GPA Tích lũy</div>
            </div>
          </div>
        ...
        {warnings.length > 0 && (
            <div className="bg-red-50 border border-red-200 rounded-2xl p-4 flex items-start gap-3">
              ...
              <div>
                ...
                {warnings.map((w, i) => <p key={i} className="text-sm text-red-600">• {w}</p>)}
              </div>
            </div>
        )}
        ...
        <div className="space-y-4">
          {Object.entries(semMap).sort(([a], [b]) => a.localeCompare(b)).map(([sem, rows]) => {
            ...
            const sg = sc > 0
                ? (rows.reduce((s, c) => s + Number(c.gpa_points) * Number(c.credits), 0) / sc).toFixed(2)
                : '—';
            return (
                <div key={sem} className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                  ...
                  <div className="divide-y divide-slate-50">
                    {rows.map(c => (
                        <div key={c.enrollment_id} className="px-5 py-3 flex items-center gap-4 hover:bg-slate-50 transition-colors">
                          <span className="text-xs font-mono font-bold text-slate-400 w-14 shrink-0">{c.course_code}</span>
