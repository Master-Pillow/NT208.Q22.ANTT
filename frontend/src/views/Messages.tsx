...
import {
  ...
  MoreVertical, Phone, Video, MessageSquare,
} from 'lucide-react';
...
const SAMPLE_MESSAGES: Message[] = [
  { id: 1, text: 'Hello Professor, I have a question regarding the recent assignment in Advanced Calculus.', time: '10:30 AM', isMe: false },
  { id: 2, text: "Hi, sure. What's your question?", time: '10:32 AM', isMe: true, status: 'read' },
  { id: 3, text: "I am having trouble with the third problem. Specifically, finding the limit of the series. The textbook example isn't very clear to me.", time: '10:34 AM', isMe: false },
  { id: 4, text: "No problem. Let's look at it together. Have you tried using the ratio test instead of the root test? It often simplifies the expression significantly for factorials.", time: '10:38 AM', isMe: true, status: 'read' },
  { id: 5, text: "Also, remember condition #2 from lecture 4. I've attached the slides for your reference.", time: '10:39 AM', isMe: true, status: 'read', attachment: 'Lecture_4_Series.pdf' },
  { id: 6, text: 'Oh, I see. I was stuck trying to apply the root test. Let me try the ratio test right now.', time: '10:41 AM', isMe: false },
  { id: 7, text: 'Thank you Professor, I will submit it tonight.', time: '10:42 AM', isMe: false },
];

export const Messages: React.FC<MessagesProps> = ({ initialContact }) => {
  const [chats, setChats] = useState<Chat[]>(DEFAULT_CHATS);
  const [activeChatId, setActiveChatId] = useState<number>(1);
  const [messagesByChat, setMessagesByChat] = useState<Record<number, Message[]>>({ 1: SAMPLE_MESSAGES });
  const [inputValue, setInputValue] = useState('');
  const [searchQuery, setSearchQuery] = useState('');
  const messagesEndRef = useRef<HTMLDivElement>(null);

  // Khi có initialContact từ ClassList, tự động mở/tạo chat với người đó
  useEffect(() => {
    if (!initialContact) return;

    // Kiểm tra xem chat với sinh viên này đã tồn tại chưa
    const existingChat = chats.find(
        (c) => c.idNumber === initialContact.mssv || c.name === initialContact.name
    );
    ...
    if (existingChat) {
      ...
      // Nếu chưa có → tạo chat mới và đặt lên đầu danh sách
      const newId = Date.now();
      const newChat: Chat = {
        id: newId,
        name: initialContact.name,
        idNumber: initialContact.mssv,
        avatar: (initialContact.id % 70) || 1,
  ...
  const handleSend = () => {
    ...
    setChats((prev) =>
        prev.map((c) =>
            c.id === activeChatId
                ? { ...c, lastMessage: inputValue.trim(), time: timeStr }
                : c
        )
    );
          ...
          <div className="flex-1 overflow-y-auto">
            {filteredChats.map((chat) => (
                <div
                    ...
                    className={cn(
                        'p-4 border-b border-slate-100 cursor-pointer transition-colors flex items-start gap-4 relative',
                        activeChatId === chat.id ? 'bg-primary/5' : 'hover:bg-slate-50'
                    )}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start mb-1">
                      ...
                      <span className="text-[10px] text-slate-400 font-medium shrink-0 ml-1">{chat.time}</span>
                    </div>
              ...
              <>
                ...
                <div className="h-20 border-b border-slate-100 flex items-center justify-between px-6 bg-white shrink-0">
                  <div className="flex items-center gap-4">
                    <div className="relative">
                      <img
                          src={`https://i.pravatar.cc/100?img=${activeChat.avatar}`}
                          alt={activeChat.name}
                                ...
                                {msg.attachment ? (
                                    ...
                                    >
                                      ...
                                      <div>
                                        ...
                                        <p className={cn('text-[10px]', msg.isMe ? 'text-white/70' : 'text-slate-400')}>
                                          PDF Document • 2.4 MB
                                        </p>
                                      </div>
                                    </div>
                ...
                <div className="p-4 border-t border-slate-100 bg-white shrink-0">
                  <div className="flex items-end gap-3 bg-slate-50 border border-slate-200 rounded-2xl p-2 focus-within:ring-2 focus-within:ring-primary/20 focus-within:border-primary/30 transition-all shadow-sm">
                    ...
                    <button
                        ...
                        className="p-2.5 bg-primary hover:bg-primary/90 disabled:bg-slate-200 disabled:text-slate-400 text-white rounded-xl shadow-md transition-colors cursor-pointer shrink-0 group"
                    >
